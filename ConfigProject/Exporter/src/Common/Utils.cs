...
namespace CSFramework
{
    public static class Utils
    {
        ...
        public static List<FileInfo> GetFileList (string path, Dictionary<string, bool> except_extension_dict = null)
        {
            ...
            foreach (var sub_dir in sub_dir_list)
                file_list.AddRange(GetFileList(sub_dir.FullName));

            var file_arr = dir_info.GetFiles();
            foreach (var file in file_arr)
            {
                var except = false;
                if (except_extension_dict != null)
                    except_extension_dict.TryGetValue(file.Extension, out except);
