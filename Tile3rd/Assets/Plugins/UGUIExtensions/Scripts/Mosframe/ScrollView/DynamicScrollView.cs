 ...
 */
 ...
 namespace Mosframe {
    ...
    public abstract class DynamicScrollView : UIBehaviour {
        ...
        public int             firstScrollToIndex = -1;
        ...
        public void scrollByItemIndex ( int itemIndex ) {
            ...
            var pos = itemLen * itemIndex - (this.viewportSize - this.itemSize) * 0.5f;
            if (pos > (this.contentSize - this.viewportSize)) pos = this.contentSize - this.viewportSize;
        ...
        // 这个方法必须在Start之前调用
        public void SetItemPrototype<T> (GameObject item) where T : MonoBehaviour, IDynamicScrollViewItem
        {
            if (null == item || null == item.GetComponent<RectTransform>())
            {
                Debug.LogError("Can not set null item prototype !");
                return;
            }
			...
			var prototypeItemRect = item.GetComponent<RectTransform>();
            if (this.direction == Direction.Horizontal)
            {
                prototypeItemRect.anchorMin = new Vector2(0.0f, 0.0f);
                prototypeItemRect.anchorMax = new Vector2(0.0f, 1.0f);
                prototypeItemRect.pivot = new Vector2(0.0f, 0.5f);
            }
            else
            {
                prototypeItemRect.anchorMin = new Vector2(0.0f, 1.0f);
                prototypeItemRect.anchorMax = new Vector2(1.0f, 1.0f);
                prototypeItemRect.pivot = new Vector2(0.5f, 1.0f);
            }
        ...
        protected virtual IEnumerator onSeedData() {
            ...
            this.scrollRect.elasticity = 0.15f;

            if (firstScrollToIndex > 0 && firstScrollToIndex < totalItemCount)
                scrollByItemIndex(firstScrollToIndex);
        }
	    ...
	    private void Update () {
		    ...
		    while( this.contentAnchoredPosition - this.prevAnchoredPosition  < -this.itemSize * 3 ) {
