...
namespace UnityEngine.UI.Extensions
{
	...
	/// </summary>
	[Serializable]
	[AddComponentMenu("Layout/Extensions/Flexible Layout Group")]
	public class FlexibleLayoutGroup : LayoutGroup
	{
		public int RowMinChildCount;
		public int MaxRows;
		...
		public float SetLayout(float width, int axis, bool layoutInput)
		{
			...
			int left = 0;
			var childCountRow = RowMinChildCount;
			if (rectChildren.Count / (float)MaxRows > RowMinChildCount)
			{
				childCountRow = rectChildren.Count / MaxRows;
				left = rectChildren.Count % MaxRows;
			}
			...
			for (var i = 0; i < rectChildren.Count; i++)
			{
				...
				if (childCount > childCountRow && (left <= 0 || childCount > childCountRow + 1))
				{
					left--;
					childCount = 1;
					currentRow++;
